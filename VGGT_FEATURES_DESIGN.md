# VGGT-Omega features for Regular CUT3R: proposal (2026-09-07)

Status: PROPOSAL, nothing implemented. Evidence base: VGGT_FEATURES_LITERATURE.md.
Scope: plain CUT3R (augfull_lr1e5 = "Regular CUT3R"), NOT the PoseGRU.

## Idea
DROID episodes carry two static exterior cameras that CUT3R never sees. CUT3R
processes only the wrist stream, has no fixed reference, and drifts (GT rays:
ATE .0134 vs .0759 -> pose is the prize). VGGT-Omega given [wrist_t0, ext1,
ext2] registers the wrist against static anchors in one forward; on the smoke
episode its poses match GT-ray-conditioned CUT3R (.0061 vs .0070). Feed that
into CUT3R two ways, both cached offline so CUT3R still runs online:
- Arm P: per-frame anchored wrist pose through the trained ray-map channel.
- Arm L: t=0 scene latent (3 x 17 camera+register tokens) as extra decoder tokens.

## Stage 0: exterior frames
Public bucket gs://gresearch/robotics/droid_raw/1.0.1 (HTTPS reachable from
login nodes; manifest.json maps uuid -> path). Two MP4s per episode (~3.5 MB
each, 1280x720, 128 frames, frame index == store frame index, verified
pixel-exact on RAIL+80edfcb1). Download on a login node in chunks (300 s CPU
cap per process; curl is I/O bound), decode frame 0 on a compute node.
Scope steps: 12 smoke scenes + RAIL episode -> 4292 test -> 38633 train.

## Stage 1: VGGT-Omega cache (env /home/koc/koc821022/.conda/envs/vggt-omega)
Wrist frame FIRST (VGGT world frame == CUT3R world frame). Per episode npz:
- t0 tokens at aggregator layers 11/17/23, fp16, [3, 1049, 2048] each
  (set model.aggregator.cached_layer_indices to include them)
- t0 camera_and_register_tokens [3, 17, 2048]
- per-frame anchored pose: forward on [wrist0, ext1, ext2, wrist_t] -> pose_enc[3] for each t -> [T, 9]
- depth + depth_conf per forward
Cache root: /gpfs/scratch/etur59/koc821022/vggt_cache/

## Stage 2: CUT3R changes (src/CUT3R/src/dust3r/model.py, datasets/dl3dv.py, eval_pipeline/infer_and_eval_worker_ray.py)
Arm P (no new modules): dataset flag feed_vggt_ray_map: {current, prev}; ray map
from cached VGGT pose + store intrinsics via the same get_ray_map used by
gtray/prevgt; eval worker --conditioning vggt|vggt_prev reads the cache;
preflight_ckpt.py must derive the new flag. Recipe = prevgt_lr1e5 recipe.
Arm L: ctx_adapter = LayerNorm(2048) -> MLP(2048->768->768), zero-init output,
tanh gate init 0, learned positions; 51 tokens PREPENDED to f_img next to
pose_token in _decoder (model.py:1325); sliced off before heads; present in all
12 layers, both towers see them; gradient in every TBPTT chunk. Context
dropped p=0.5 per sample (masked token) so one ckpt is honest in both corners.
Sub-arm L2: + layer-17 patch tokens. Config flag vggt_ctx: {none, camreg, camreg+p17}.
Rationale (literature): token concat / additive beats cross-attn KV when the
consumer is finetuned (Idefics2, VG-LLM, GAP-MLLM, G-CUT3R); zero-init gate is
non-negotiable (Flamingo, LLaMA-Adapter, G-CUT3R ablations); linear/MLP is
enough (Honeybee, LLaVA-1.5, MM1); layers 11-18 carry geometry (RegimeVGGT).

## Stage 3: scoring order (information per GPU-hour)
1. VGGT anchored poses scored directly (no CUT3R) -> quality of the exogenous feed.
2. Arm P fed into EXISTING prevgt_lr1e5 / gtray_lr1e5 ckpts, zero training
   (noise oracle: exogenous error up to head magnitude is tolerated).
3. Control: exterior frames prepended natively through CUT3R as UNSCORED
   context views, zero training (CUT3R revisit gains .126->.113 in its paper).
4. Arm L plumbing check (bit-identical to augfull at zero-init), then
   single-episode overfit liveness (adapter grad norm > 0, loss < no-ctx overfit).
5. 10-epoch rungs from augfull checkpoint-final, 32 GPU, lr 1e-5, for Arm P and
   Arm L; baseline continued same steps as control.
6. arm_eval_for_run.sh: 12 smoke (cg_smoke_scenes_12.txt in my-da3) + RAIL ->
   430 subset -> 4292; win rule = per-scene win% vs augfull on ATE AND rpe_rot.

## Budget
| item | test (4292) | train (38633) |
|---|---|---|
| exterior MP4 download | ~30 GB | ~270 GB, delete after frame-0 extraction |
| VGGT t0 forwards | minutes | ~2 GPU-h |
| VGGT per-frame anchored forwards | ~25 GPU-h | ~215 GPU-h |
| token cache 3 layers fp16 | ~165 GB | register-only ~10 GB |

## Open decisions (user)
1. Scale for Arm P: let the finetune absorb VGGT's normalised scale (default;
   anchors fix it per episode) vs rescale to metric with calibrated exterior
   extrinsics from episode metadata (calibration, not trajectory GT).
2. Benchmark definition: exterior cameras at eval make it a rig benchmark;
   label rows accordingly.
3. Arm P failure handling when wrist close-ups do not register: hold last good
   anchored pose (attenuate, don't amputate); measure in step 1.

## Expectation
Arm P is the arm most likely to move ATE (pose priors: G-CUT3R .098->.061 Acc,
MapAnything 56->94 AUC, DA3 57->70 F1; here prevgt captured 97% of the prize).
Arm L: literature predicts a few points at most; no prior work feeds one 3D
model's latents into another reconstruction model or uses static exterior
cameras as context for a streaming one.
